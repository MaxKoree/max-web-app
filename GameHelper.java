...
public class GameHelper {
  ...
  public static List<Card> createDeck() {
    ...
    for (final Cards card : Cards.values()) {
      deck.add(card);
    }
    return deck;
  }
   ...
   */
  public static List<Card> shuffleDeck(final List<Card> deck) {
    shuffleDeck(deck, 1);
    return deck;
  }
   ...
   * @return the shuffled deck.
   */
  public static List<Card> shuffleDeck(final List<Card> deck, final int number) {
    int i = 0;
    while (i < number) {
   ...
   */
  public static List<Card> createHand(final List<Card> deck) {
    final List<Card> hand = new ArrayList<>();
    for (int i = 0; i < 10; i++) {
      final Cards c = (Cards) deck.remove(0);
      hand.add(c);
    }
    return hand;
  }
   ...
   */
  public static Cards giveCard(final List<Card> deck) {
    final Cards card = (Cards) deck.remove(0);
    return card;
  }
   ...
   */
  public static List<Cards> getPlayersCardsOnTable(final List<Player> players) {
    final List<Cards> card = new ArrayList<>();
    for (final Player player : players) {
      final Cards cardOnTable = (Cards) player.getCardOnTable();
      if (cardOnTable != null) {
   ...
   */
  public static Cards whoHasLowestCard(final List<Cards> cardsOnTable) {
    final List<Cards> cards = new ArrayList<>(cardsOnTable);
    final Cards lowestCard =
        cards.stream().min(Comparator.comparing(Card::getValue)).orElseThrow(NoSuchElementException::new);
    return lowestCard;
   ...
   */
  public static Player getPlayerWithLowestCardOnTable(final List<Player> players, final User user)
      throws NoSuchElementException {
    final List<Player> playersInGame = new ArrayList<>();
    ...
    if (playersInGame.size() > 1) {
      playerWithLowestCard = playersInGame.stream().min(Comparator.comparing(plyr -> plyr.getCardOnTable().getValue()))
          .orElseThrow(NoSuchElementException::new);
    } else {
      playerWithLowestCard = playersInGame.get(0);
    }
    System.out.println(playerWithLowestCard.getUser());
   ...
   */
  public static boolean checkPlayerCardAgainstRows(final Player player, final Game game) {
    ...
    if (!after.isEmpty()) {
      ...
      final int number = (int) (Math.random() * 4);
  ...
  /**
   * tradeCardForRow will replace the cards of a row for the players card and add the replaced cards to the players
   * score.
   *
   * @param player is the player that played the card.
   * @param row is the row which is being replaced.
   */
  public static void tradeCardForRow(final Player player, final List<Card> row) {
    for (final Card x : row) {
      player.addToScore(x);
    }
    row.clear();
   ...
   */
  public static void rowChecker(final Player player, final List<Card> after, final List<Card> row) {
   ...
   */
  public static List<Card> getRow(final int number, final Game game) throws Exception {
    if (number == 0) {
      return game.getRow0();
    } else if (number == 1) {
      return game.getRow1();
